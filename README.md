...
```
## 项目反思
我在写代码的过程中，过于专注，忽略了git的提交，有不想再去搞交互式暂存，最后只能够合并提交。
虽然写伪代码的时候感觉会很简单，但是在写代码的时候，才发现，这个项目的重点是它的运行顺序，以及作用域的问题。细节上也有一些需要注意的，例如：
1. return截断，必须要考虑return的顺序
2. if 判断的简化
3. 局部变量的生命周期
4. for 循环对外部函数的结果获取，如果没有获取到外部的函数，会发生循环冻结
